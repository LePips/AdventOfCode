        ...
        input()
            ...
            .forEach { line in
                ...
                if parts[0] == "$" {
                    ...
                    let fileNode: Node<Inode> = Node(value: .init(
                        name: parts[1].asString,
                        isDirectory: false,
                        size: parts[0].asString.asInt!
                    ))
...
extension Node<Day7.Inode> {
    ...
    func directorySizes() -> [Int] {
        children
            .filter(\.value.isDirectory)
            .reduce([size()]) { partialResult, node in
                partialResult
                    .appending(node.size())
                    .appending(node.children.filter(\.value.isDirectory).map { $0.directorySizes() }.flatMap { $0 })
            }
    }
}
